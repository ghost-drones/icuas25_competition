...
public:
  PathPlanner()
  : Node("path_planner"), support_radius_(5.0)
  {
...
private:
    ...
    icuas25_msgs::srv::PathService::Response::SharedPtr response)
  {
    ...
    octomap::point3d direcao = destination - origin;
    double distancia_total = direcao.norm();
    direcao.normalize();

    double t_min = std::max(0.0, 1.0 - (support_radius_ / distancia_total));
    
    for (double t = 1.0; t >= t_min; t -= passo / distancia_total) {
        octomap::point3d ponto_atual = origin + direcao * (t * distancia_total);

        if ((ponto_atual - support).norm() > support_radius_) continue;
